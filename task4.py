...
# Constants
m = 10  # mass in kg
b = 70  # damping coefficient in N*s/m (initial damping)
k = 1000  # spring constant (N/m)
K_p = 1000  # Proportional gain for PD controller (adjustable)
K_d = 50  # Derivative gain for PD controller (adjustable)
delta_t = 0.001  # time step in seconds
delay_steps = 30  # Number of steps to introduce as delay (adjustable)
...
# Initial conditions
z_0 = 6.6/1000  # initial position (m)
...
force_control = np.zeros(len(time))  # Control force from PD controller
...
# Simulate the system using the PD control model (force-driven position update)
for n in range(1, len(time)):
    ...
    # PD control: Proportional and Derivative terms
    # Calculate the derivative (velocity)
    if n > 1:
        velocity_actual = (z_actual[n] - z_actual[n-1]) / delta_t
        velocity_desired = (z_desired[n] - z_desired[n-1]) / delta_t
    else:
        velocity_actual = 0
        velocity_desired = 0
    
    # PD controller force
    F_control = K_p * (z_desired[n-1] - z_actual[n-1]) + K_d * (velocity_desired - velocity_actual)

    # Total force acting on the object (control + spring + sensor force)
    F_total = F_sensor + F_spring + F_control
